...
contract UACToken is IERC20, IERC20Metadata, Ownable {
    ...
    string private _name = "UAC Token";
    ...
    function batchSetTradeWhitelistBuyLimit(address[] calldata _accounts, uint256[] calldata _limits) external onlyOwner {
        require(_accounts.length == _limits.length, "UACToken: Arrays length mismatch");
        for (uint256 i = 0; i < _accounts.length; i++) {
    ...
    function setPair(address _pair, bool _state) public onlyOwner {
        require(_pair != address(0), "UACToken:ZERO_ADDRESS");
        pairs[_pair] = _state;
    }
    ...
    function setExcludeFee(address _address, bool _state) public onlyOwner {
        require(_address != address(0), "UACToken:ZERO_ADDRESS");
    ...
    function setBuyFeeReceivers(address[] calldata _receivers, uint256[] calldata _rates) external onlyOwner {
        require(_receivers.length == _rates.length, "UACToken: Arrays length mismatch");
        require(_receivers.length > 0, "UACToken: Empty arrays");
        
        // 检查总费率不超过100%
        uint256 totalRate = 0;
        for (uint256 i = 0; i < _rates.length; i++) {
            // require(_receivers[i] != address(0), "UACToken: Zero address");
            require(_rates[i] > 0 && _rates[i] <= 1000, "UACToken: Invalid rate");
            totalRate += _rates[i];
        }
        require(totalRate <= 1000, "UACToken: Total buy fee rate exceeds 100%");
    ...
    function setSellFeeReceivers(address[] calldata _receivers, uint256[] calldata _rates) external onlyOwner {
        ...
        require(totalRate <= 1000, "UACToken: Total sell fee rate exceeds 100%");
    ...
    function addBuyFeeReceiver(address _receiver, uint256 _rate) external onlyOwner {
        // require(_receiver != address(0), "UACToken: Zero address");
        require(_rate > 0 && _rate <= 1000, "UACToken: Invalid rate"); // 最大100%
    ...
    function updateBuyFeeReceiver(uint256 _index, address _receiver, uint256 _rate) external onlyOwner {
        require(_index < buyFeeReceivers.length, "UACToken: Index out of bounds");
        // require(_receiver != address(0), "UACToken: Zero address");
        require(_rate > 0 && _rate <= 1000, "UACToken: Invalid rate");
    ...
    function updateSellFeeReceiver(uint256 _index, address _receiver, uint256 _rate) external onlyOwner {
        require(_index < sellFeeReceivers.length, "UACToken: Index out of bounds");
        // require(_receiver != address(0), "UACToken: Zero address");
        require(_rate > 0 && _rate <= 1000, "UACToken: Invalid rate");
   ...
   function _transfer(address from, address to, uint256 amount) internal {
        ...
        // 检查交易是否整体开放
        require(tradeToPublic, "UACToken: not open");
        ...
        if (isSwap) {
            ...
            if (globalWhitelist[from] || globalWhitelist[to]) {
                ...
                // 不是全局白名单就要校验交易币对是否开启
                if (pairs[from]) {
                    require(pairsEnabled[from], "UACToken: Pair not enabled");
                }
                if (pairs[to]) {
                    require(pairsEnabled[to], "UACToken: Pair not enabled");
                }    
                // 1. 检查交易开关
                if (!tradingEnabled) {
                    // 交易开关关闭时，只允许交易白名单用户交易
                    require(tradeWhitelist[from] || tradeWhitelist[to], "UACToken: Trading disabled");
