...
contract INFOErc20 is IERC20, IERC20Metadata, Ownable {
    ...
    address public infoWBNBPairAddr;
    ...
    function batchSetTradeWhitelistBuyLimit(address[] calldata _accounts, uint256[] calldata _limits) external onlyOwner {
        require(_accounts.length == _limits.length, "INFO: Arrays length mismatch");
        for (uint256 i = 0; i < _accounts.length; i++) {
    ...
    function setPair(address _pair, bool _state) public onlyOwner {
        require(_pair != address(0), "INFO:ZERO_ADDRESS");
        pairs[_pair] = _state;
    }
    ...
    function setExcludeFee(address _address, bool _state) public onlyOwner {
        require(_address != address(0), "INFO:ZERO_ADDRESS");
    ...
    function setBuyFeeReceivers(address[] calldata _receivers, uint256[] calldata _rates) external onlyOwner {
        require(_receivers.length == _rates.length, "INFO: Arrays length mismatch");
        require(_receivers.length > 0, "INFO: Empty arrays");
        
        // 检查总费率不超过100%
        uint256 totalRate = 0;
        for (uint256 i = 0; i < _rates.length; i++) {
            // require(_receivers[i] != address(0), "INFO: Zero address");
            require(_rates[i] > 0 && _rates[i] <= 1000, "INFO: Invalid rate");
            totalRate += _rates[i];
        }
        require(totalRate <= 1000, "INFO: Total buy fee rate exceeds 100%");
    ...
    function setSellFeeReceivers(address[] calldata _receivers, uint256[] calldata _rates) external onlyOwner {
        ...
        require(totalRate <= 1000, "INFO: Total sell fee rate exceeds 100%");
    ...
    function addBuyFeeReceiver(address _receiver, uint256 _rate) external onlyOwner {
        // require(_receiver != address(0), "INFO: Zero address");
        require(_rate > 0 && _rate <= 1000, "INFO: Invalid rate"); // 最大100%
    ...
    function updateBuyFeeReceiver(uint256 _index, address _receiver, uint256 _rate) external onlyOwner {
        require(_index < buyFeeReceivers.length, "INFO: Index out of bounds");
        // require(_receiver != address(0), "INFO: Zero address");
        require(_rate > 0 && _rate <= 1000, "INFO: Invalid rate");
    ...
    function updateSellFeeReceiver(uint256 _index, address _receiver, uint256 _rate) external onlyOwner {
        require(_index < sellFeeReceivers.length, "INFO: Index out of bounds");
        // require(_receiver != address(0), "INFO: Zero address");
        require(_rate > 0 && _rate <= 1000, "INFO: Invalid rate");
   ...
   function _transfer(address from, address to, uint256 amount) internal {
        ...
        if (pairs[from]){
            ...
            require(transTradingEnabled,"INFO: tradingEnabled not enable");
            _standardTransfer(from, to, amount); 
        }
    ...
    function setDepositContract(address _depositContract) public onlyOwner {
        require(_depositContract != address(0),"0 address invalid");
        depositContract = _depositContract;
    }
